...
export class DBService {
  ...
  async createFile(title: string, slug: string, name: string, content: string) {
    try {
      ...
      const file = new File([fileContent], `${slug}.txt`, {
        type: "text/plain",
      });

      const uploadFile = await this.buckets.createFile(
      ...
      const document = await this.databses.createDocument(
        conf.appwriteBucketID,
